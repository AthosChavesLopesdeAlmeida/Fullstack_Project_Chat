...
const Page = () => {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    if (!ok) {
      setError(data?.message || 'Erro ao logar usuário')
      return
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#14151F] px-4">
      <div className="w-full max-w-sm">
        <div className="mb-6 text-center">
          <h3 className="text-xl font-semibold tracking-tight text-[#EDEDF4]">Entre na sua conta</h3>
        </div>
        ...
        <form onSubmit={(e) => handleSubmit(e)} className="flex flex-col gap-4 rounded-2xl border border-[#2A2C3D] bg-[#1A1B26] p-6">
          ...
          <Link href={'/register'} className="text-center">
            <p className="text-xs text-[#8B8FA3] transition-colors hover:text-[#2DD4BF]">Não tem uma conta? Crie uma aqui</p>
          </Link>
